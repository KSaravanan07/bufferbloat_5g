...
LteRlcUm::LteRlcUm ()
  : m_maxTxBufferSize (10 * 1024),
    ...
    m_enbdynRlc(false),
    m_interval(MilliSeconds(1).GetNanoSeconds()),
    m_dequed_bytes(0),
    m_last_time(Seconds(0.4).GetNanoSeconds()),
...
LteRlcUm::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteRlcUm")
    ...
    .AddAttribute ("ReorderingTimer",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("EnableDynamicRLC",
                    "Set true to enable dynamic RLC.",
                   BooleanValue(false),
                   MakeBooleanAccessor(&LteRlcUm::m_enbdynRlc),
                   MakeBooleanChecker())
    .AddAttribute ("DyanmicRLCInterval",
                   "Interval after which RLC buffer needs to be updated in milliseconds",
                   UintegerValue(MilliSeconds(1).GetNanoSeconds()),
                   MakeUintegerAccessor(&LteRlcUm::m_interval),
...
LteRlcUm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  ...
  if (m_txBufferSize + p->GetSize () <= m_limit)
    {
      /** Store PDCP PDU */
  ...
  else
    {
      ...
      NS_LOG_LOGIC ("MaxTxBufferSize = " << m_limit);
      NS_LOG_LOGIC ("txBufferSize    = " << m_txBufferSize);
      NS_LOG_LOGIC ("packet size     = " << p->GetSize ());
      m_txDropTrace (p);
      has_packet_dropped = true;
    }
...
LteRlcUm::DoNotifyTxOpportunity (LteMacSapUser::TxOpportunityParameters txOpParams)
{
  ...
  bytes_transmitted+=firstSegment->GetSize();
  NS_LOG_LOGIC ("txBufferSize      = " << m_txBufferSize );
          ...
          if (m_txBuffer.size () > 0)
            {
              ...
              //bytes_transmitted+=m_txBuffer.begin()->m_pdu->GetSize ();
            }
          NS_LOG_LOGIC ("        Next segment size = " << nextSegmentSize);
  ...
  if(m_enbdynRlc)
  DynamicRLCQueueLimit(bytes_transmitted); 
}
...
void LteRlcUm::DynamicRLCQueueLimit(int64_t bytes_transmitted){
  ...
  NS_LOG_LOGIC("Limits : " << m_limit);
    ...
    if(m_interval >= (Simulator::Now().GetNanoSeconds() - m_last_time)){   // initialize global values after every m_interval time
    m_last_time = Simulator::Now().GetNanoSeconds(); 
...
LteRlcUm::DoReceivePdu (LteMacSapUser::ReceivePduParameters rxPduParams)
{
  ...
  //std::cout<< m_rnti <<"   "<< (uint32_t) m_lcid<< "    " << rxPduParams.p->GetSize ()<<std::endl;
