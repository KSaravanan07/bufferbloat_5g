...
NS_LOG_COMPONENT_DEFINE ("solution");
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("singleUeTopology",
                ...
                singleUeTopology);
  cmd.AddValue ("useDynRlc",
                "Whether to use dynamic RLC buffer",
                useDynRlc);
 ...
 // Config::SetDefault ("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue (50*1024));
  //Config::SetDefault ("ns3::LteRlcUm::", UintegerValue (999999999));
  Config::SetDefault ("ns3::LteRlcUm::EnableDynamicRLC", BooleanValue(useDynRlc));
...
//  nrHelper->SetChannelConditionModelAttribute ("UpdatePeriod", TimeValue (MilliSeconds (0)));
  ...
  serverApps2.Add (dlPacketSinkHelper2.Install (ueNodes.Get (0)));
  ...
  dlClient2.SetAttribute ("RemotePort", UintegerValue (dlPort2));
  ...
  dlClient2.SetAttribute ("Interval", TimeValue (Seconds (1.0 / 100)));
  ...
  for (uint32_t i = 0; i < ueNodes.GetN (); ++i)
    {
      ...
      clientApps2.Add (dlClient2.Install (remoteHost));
  ...
  serverApps2.Start (Seconds (udpAppStartTime));
  clientApps.Start (Seconds (udpAppStartTime));
  clientApps2.Start (Seconds (udpAppStartTime + 1));
